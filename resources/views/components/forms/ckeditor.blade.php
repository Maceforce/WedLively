...
<div class="{{ $errors->first($model) ? 'ckeditor-has-error' : '' }}">
    ...
    {{-- Label --}}
    <label for="text-input-component-id-{{ $model }}" class="block text-[0.8125rem] font-medium {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-gray-700 dark:text-white' }}">{{ $label }}</label>
