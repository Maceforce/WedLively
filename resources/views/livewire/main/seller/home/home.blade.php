        ...
        <div class="mx-auto max-w-7xl">
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="mt-5 flex justify-between lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4">
					...
					{{-- Switch to buying --}}
					<!--<span class="block">
                        <a href="{{ url('/') }}" class="inline-flex items-center  border border-gray-300 bg-white px-4 py-2 text-[13px] font-semibold text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2 dark:bg-zinc-800 dark:border-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-900 dark:focus:ring-offset-zinc-900 dark:focus:ring-zinc-900">
					...
					<span class="sm:ltr:ml-3 sm:rtl:mr-3">
						<a href="{{ url('create') }}" class="inline-flex items-center  border border-red-400 bg-red-400 px-4 py-2 text-[13px] font-semibold text-white hover:text-red-500 focus:text-red-400 hover:bg-transparent transition-colors duration-300">
		...
		<div class="mt-4 grid grid-cols-12 gap-4 sm:mt-5 lg:mt-6 sm:gap-y-5 lg:gap-y-6 sm:gap-x-6 lg:gap-x-12">
			...
			<div class="col-span-12 xl:col-span-8">
				<div class="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 sm:gap-5 lg:gap-6">
					...
					{{-- Earnings --}}
					<div class=" border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-sm p-3 flex flex-col justify-between">
						<div class="flex justify-between space-x-1 rtl:space-x-reverse">
							...
							<div class="w-9 h-9 flex items-center justify-center  bg-zinc-100 dark:bg-zinc-700">
					...
					<div class=" border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-sm p-3 flex flex-col justify-between">
						<div class="flex justify-between space-x-1 rtl:space-x-reverse">
							<p class="text-lg font-bold text-zinc-700 dark:text-zinc-100">
								{{ readable_number($total_reach) }}
							</p>
							<div class="w-9 h-9 flex items-center justify-center  bg-zinc-100 dark:bg-zinc-700">
								<svg class="h-5 w-5 shrink-0 text-zinc-600 dark:text-zinc-300" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g><path fill="none" d="M0 0H24V24H0z"></path><path d="M5 3v16h16v2H3V3h2zm14.94 2.94l2.12 2.12L16 14.122l-3-3-3.94 3.94-2.12-2.122L13 6.88l3 3 3.94-3.94z"></path></g></svg>
							</div>
						</div>
						<p class="mt-1 text-[13px] text-gray-500 dark:text-zinc-300 font-medium">@lang('messages.t_total_reach')</p>
					</div>
			...
			<div class="col-span-12 xl:col-span-4">
				<div class=" border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-sm px-4 py-4 sm:px-5">
						...
						@forelse ($latest_messages as $message)
							<a href="{{ url('inbox', $message['uid']) }}" class="flex cursor-pointer items-center justify-between space-x-2 rtl:space-x-reverse">
								...
								<div class="flex items-center space-x-3 rtl:space-x-reverse">
									...
									{{-- Avatar --}}
									<div class="h-10 w-10">
										<img class=" object-contain" src="{{ $message['avatar'] }}" alt="{{ $message['username'] }}">
									</div>
								...
								{{-- Go to conversation --}}
								<div class="hover:text-red-400 focus:text-red-400 text-slate-500">
					...
					<table class="w-full text-left border-spacing-y-[10px] border-separate sm:mt-2">
						<thead class="">
							<tr class="bg-slate-200 dark:bg-zinc-600">
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300     rtl:text-right">@lang('messages.t_gig')</th>
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_profit')</th>
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_expected_delivery_date')</th>
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_status')</th>
							</tr>
						</thead>
						<thead>
							@forelse ($latest_orders as $order)
								<tr class="intro-x shadow-sm bg-white dark:bg-zinc-800  h-16" wire:key="freelancer-dashboard-latest-order-{{ $order->uid }}">

									{{-- Gig --}}
									<td class="px-5 py-3     w-72 rtl:text-right">
										<div class="flex items-center space-x-3 rtl:space-x-reverse">
		
											{{-- Thumbnail --}}
											<div class="h-10 w-10">
												<img class="w-full h-full  object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($order->gig->thumbnail) }}" alt="{{ $order->gig->title }}">
											</div>
											...
											<div>
												...
												{{-- Title --}}
												<a href="{{ url('service', $order->gig->slug) }}" class="font-medium whitespace-nowrap truncate block max-w-sm hover:text-gray-500 dark:text-white text-sm" title="{{ $order->gig->title }}">
													{{ $order->gig->title }}
												</a>
									...
									<td class="px-5 py-3     text-center">
										@if ($order->expected_delivery_date)
											<span class="text-sm whitespace-nowrap font-normal text-zinc-600 dark:text-zinc-300">{{ format_date($order->expected_delivery_date, 'F j, Y h:i A') }}</span>
									...
									{{-- Status --}}
									<td class="px-5 py-3     w-40 text-center">
										@if ($order->refund && $order->refund->status === 'pending')
											<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-red-50 text-red-800 dark:text-red-500">
												{{ __('messages.t_dispute_opened') }}
											</span>
										@elseif ($order->order?->invoice && $order->order->invoice->status === 'pending')
											<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-amber-50 text-amber-800 dark:text-amber-500">
										...
										@else
											@switch($order->status)
												...
												{{-- Pending --}}
												@case('pending')
													<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-amber-50 text-amber-800 dark:text-amber-500 tracking-wide">
												...
												{{-- Delivered --}}
												@case('delivered')
													<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-green-50 text-green-800 dark:text-green-500">
												...
												{{-- Proceeded --}}
												@case('proceeded')
													<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-blue-50 text-blue-800 dark:text-blue-500">
												...
												{{-- Canceled --}}
												@case('canceled')
													<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-gray-50 text-gray-800 dark:text-gray-500">
					...
					<table class="w-full text-left border-spacing-y-[10px] border-separate sm:mt-2">
						<thead class="">
							<tr class="bg-slate-200 dark:bg-zinc-600">
								...
								{{-- Project --}}
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300     rtl:text-right">@lang('messages.t_project')</th>
								
								{{-- Status --}}
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_status')</th>
								
								{{-- Your proposal --}}
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_my_proposal')</th>
								
								{{-- Awarded date --}}
								<th class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300    ">@lang('messages.t_awarded_date')</th>

							</tr>
						</thead>
						<thead>
							@foreach ($latest_awarded_projects as $project)
								<tr class="intro-x shadow-sm bg-white dark:bg-zinc-800  h-16" wire:key="freelancer-dashboard-latest-projects-{{ $project->uid }}">

									{{-- Project --}}
									<td class="px-5 py-3     w-60 rtl:text-right">
										<a href="{{ url('project/' . $project->pid . '/' . $project->slug) }}" class="font-medium whitespace-nowrap truncate block max-w-sm hover:text-red-400 dark:text-white text-sm" title="{{ $project->title }}">
											{{ $project->title }}
										</a>
									</td>
									...
									<td class="px-5 py-3     w-40 text-center">
										@switch($project->status)
											...
											@case('pending_final_review')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-amber-50 text-amber-800 dark:text-amber-500 tracking-wide whitespace-nowrap">
											...
											{{-- Active --}}
											@case('active')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-blue-50 text-blue-800 dark:text-blue-500 tracking-wide whitespace-nowrap">
											...
											{{-- Completed --}}
											@case('completed')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-emerald-50 text-emerald-800 dark:text-emerald-500 tracking-wide whitespace-nowrap">
											...
											{{-- Incomplete --}}
											@case('incomplete')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-red-50 text-red-800 dark:text-red-500 tracking-wide whitespace-nowrap">
											...
											@case('under_development')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-purple-50 text-purple-800 dark:text-purple-500 tracking-wide whitespace-nowrap">
											...
											{{-- Closed --}}
											@case('closed')
												<span class="inline-flex dark:bg-transparent items-center px-2.5 py-1  text-xs font-medium bg-gray-50 text-gray-800 dark:text-gray-500 tracking-wide whitespace-nowrap">
