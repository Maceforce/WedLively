<div class="gig-card" x-data="window._{{ $gig->uid }}" dir="{{ config()->get('direction') }}">
    <div class="bg-white dark:bg-zinc-800 shadow-sm ring-1 ring-gray-200 dark:ring-zinc-800">
        ...
        <div class="px-4 pb-4 mt-4">
                    ...
                    <a href="{{ url('profile', $gig->owner->username) }}" target="_blank" class="flex-shrink-0 group block">
                        <div class="flex items-center">
                            <span class="inline-block relative">
                                <img class="h-9 w-9  object-cover lazy shadow-sm" src="{{ placeholder_img() }}" data-src="{{ src($gig->owner->avatar) }}" alt="{{ $gig->owner->username }}">
                            </span>
                        <div class="ltr:ml-3 rtl:mr-3">
                            <div class="text-gray-800 dark:text-gray-200 hover:text-gray-900 dark:hover:text-gray-300 flex items-center mb-0.5 font-extrabold tracking-wide text-[13px]">
                                ...
                                @if ($gig->owner->status === 'verified')
                                    ...
                                    <div id="tooltip-account-verified-{{ $gig->uid }}" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900  shadow-sm opacity-0 tooltip dark:bg-gray-700">
            ...
            {{-- Title --}}
            <a href="{{ url('service', $gig->slug) }}" class="gig-card-title font-semibold dark:text-gray-100 hover:text-red-500 text-gray-800 dark:hover:text-white mb-5 !overflow-hidden">
        ...
        <div class="px-3 py-2 border-t border-gray-50 dark:border-zinc-700 text-right sm:px-4  flex justify-between bg-red-400">
            ...
            <div class="flex items-center">
                ...
                {{-- Add to favorite --}}
                <button @auth @if ($favorite) wire:click="removeFromFavorite('{{ $gig->uid }}')" wire:target="removeFromFavorite('{{ $gig->uid }}')" @else wire:click="addToFavorite('{{ $gig->uid }}')" wire:target="addToFavorite('{{ $gig->uid }}')" @endif wire:loading.attr="disabled" @endauth @guest x-on:click="loginToAddToFavorite" @endguest data-tooltip-target="tooltip-add-to-favorites-{{ $gig->uid }}" class="h-8 w-8  flex items-center justify-center -ml-1 focus:outline-none visited:outline-none text-white">
                ...
                <div id="tooltip-add-to-favorites-{{ $gig->uid }}" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900  shadow-sm opacity-0 tooltip dark:bg-gray-700">
                    @if ($favorite)
            ...
            <div class="gig-card-price">
                <p class="text-white dark:!text-zinc-200 mb-1 text-sm">{{ __('messages.t_starting_at') }}</p>
                <p class=" ltr:text-right rtl:text-left dark:!text-white text-white mb-0 text-md font-bold">@money($gig->price, settings('currency')->code, true)</p>
            </div>
